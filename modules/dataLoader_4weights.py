...
class dataLoader(object):
        ...
        if hasattr(self.user, 'nugen_nue') and hasattr(self.user, 'nugen_numu') and self.break_energy > 0:
            if self.user.nugen_nue != None and self.user.nugen_numu != None:
                ...
                self.nue['fixed_baseline']  = mf.mergeDictionaries(genie_dict = self.nue['baseline'], 
                                                                   ...
                                                                   weight_keys = [self.we, self.we_pi, self.wmu, self.wmu_pi],
    ...
    def getNuHistogram(self, inparams = {}, weight_power = 1., use_kde=False, return_weights = False):
        ...
        # Making an event with zero flux by default on all lists. Avoids issues with empty expectations.
        if len(nue_cc['energy']) == 0:
            for one_key in nue_cc.keys():
                nue_cc[one_key] = np.array([0.])
            nue_cc['ptype'] = np.array([12])
            nue_cc['ma_qe'] = np.array([[0., 0.]])
            nue_cc['ma_res'] = np.array([[0., 0.]])
        if len(numu_cc['energy']) == 0:
            for one_key in numu_cc.keys():
                numu_cc[one_key] = np.array([0.])
            numu_cc['ptype'] = np.array([14])
            numu_cc['ma_qe'] = np.array([[0., 0.]])
            numu_cc['ma_res'] = np.array([[0., 0.]])
        if len(nutau_cc['energy']) == 0:
            for one_key in nutau_cc.keys():
                nutau_cc[one_key] = np.array([0.])
            nutau_cc['ptype'] = np.array([16])
            nutau_cc['ma_qe'] = np.array([[0., 0.]])
            nutau_cc['ma_res'] = np.array([[0., 0.]])
        ...
        # NuE produced as NuE
        nue_nue_ccflux    =  (nue_cc[self.we] + params['nu_pi_scale']*nue_cc[self.we_pi])*params['norm_e']*nue_cc['energy']**params['gamma']#*\
        ...
        numu_nue_ccflux   = (numu_cc[self.we]+params['nu_pi_scale']*numu_cc[self.we_pi])*params['norm_e']*numu_cc['energy']**params['gamma']#*\
        ...
        nutau_nue_ccflux    = (nutau_cc[self.we] + params['nu_pi_scale']* nutau_cc[self.we_pi])*params['norm_e']*nutau_cc['energy']**params['gamma']#*\
        ...
        nue_ncweight  = (nue_nc[self.we]+ params['nu_pi_scale']*nue_nc[self.we_pi])*params['norm_e']*nue_nc['energy']**params['gamma']#*\
	...
	#print neutrino_histograms.shape()
	for key in neutrino_histograms:
		print key
                print np.sum(neutrino_histograms[key][:,:,0])
                #print np.sum(neutrino_histograms[key][:,:,1])
    ...
    def __init__(self, 
                 ...
                 user         = 'quickstart', 
                 LEaxis       = [],
                 weight_keys  = ['weight_e_k','weight_e_pi', 'weight_mu', 'weight_mu_pi'], 
                 break_energy = -1,
                 extra_cuts   = {},
                 table_nbins  = -1,
		 ...
		 #pmodel = 'GH',
		 #crmodel = 'DPMJET-III',
                 #sysfile  = 'DRAGON_extended'+pmodel+'.'+crmodel+'.sysfile.py'):
		 sysfile = 'DRAGON_Extended_auto_sysfile_test.pckl'):
        ...
        self.we_pi           = weight_keys[1]
        self.wmu             = weight_keys[2]
        self.wmu_pi          = weight_keys[3]
