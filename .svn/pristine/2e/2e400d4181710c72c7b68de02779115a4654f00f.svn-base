...
class likelihoods():
    ...
    #------------------------------------------------------------------
    # Set up the data histogram. This histogram is flattened in order
    #  to make the looping later on a bit simpler to handle.
    #------------------------------------------------------------------
    ...
    #------------------------------------------------------------------
    # Set up the MC histogram. Each histogram is flattened in order
    #  to make the looping later on a bit simpler to handle.
    #  The values in each bin should correspond to the weight-per-event
    #  in that bin, NOT the total weight in that bin!
    #
    ...
    def GetSinglePlots(self):
        ...
        shape.extend(list(self.shape))
        shape = tuple(shape)

        return numpy.reshape(result, shape)
    ...
    #  with the bounds set in the normal way, so the positive-definiteness
    #  of the rates is enforced in the getLLH_barlow_bin method.
    #------------------------------------------------------------------
    def GetLLH(self, llh_type):
        self.bestfit_plots = self.unweighted_histograms[:]
        self.current_bin = 0
    ...
    #  correlations between bins. This likely leads to a somewhat worse
    #  limit than you might get otherwise, but at least its conservative.
    #  
    ...
    def getLLH_barlow_bin(self, Ai):
        ...
        # This is the standard Poisson LLH comparing data to the total weighted MC
        ...
        #  in the MC sample for this bin) using the observed MC events as a constraint.
        cut = Ai > 0
        LLH += numpy.sum(numpy.dot(ai[cut], numpy.log(Ai[cut])) - numpy.sum(Ai[cut]))

        # This is simply a normalization term that helps by centering the LLH near 0
