...
def runScan(options):
    ...
    job_script = os.path.join(os.path.dirname(os.path.realpath(__file__)),'oscFit_oneJob.py')
    ...
    if options.MODE == 'uge_farm':
        ...
        if len(options.JOBS) == 0:
            total_jobs = np.ceil(total_jobs*1./options.FARM_MULTIPLICITY)
            job_array =  '1-'+"%i" % total_jobs
            qsub_line = '  '.join(['qsub -t',job_array,'./oscFit_farmScript.sh',
                                   job_script,"%i" % options.FARM_MULTIPLICITY,options.NAME])
            print qsub_line
            os.system(qsub_line)
        else:
...
if __name__ == '__main__':
    ...
    parser.add_option('-j', '--jobs',type='string',action='callback', callback=foo_callback,
                      default=[], dest="JOBS")
    parser.add_option('-f', '--farm_multiplicity',type='int',
                      default=2, dest="FARM_MULTIPLICITY",
                      help='Select how many points to calculate per farm job')
